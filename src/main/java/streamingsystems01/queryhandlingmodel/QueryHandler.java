package streamingsystems01.queryhandlingmodel;

import streamingsystems01.MovingItem;
import streamingsystems01.queryhandlingmodel.predefined.Query;
import streamingsystems01.implemented.MovingItemDTO;
...
public class QueryHandler implements Query {
    ...
    /**
     * Returns all moving items as a collection.
     *
    ...
    public Enumeration<MovingItemDTO> getMovingItemsAtPosition(int[] position) {
        Collection<MovingItemDTO> movingItemDTOsAtPosition = getAllMovingItemsAsCollection().stream()
                .filter((MovingItem eachMovingItem) -> eachMovingItem.getLocation() == position)
                .toList();
